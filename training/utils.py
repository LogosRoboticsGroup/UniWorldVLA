...
def adapt_raw_scene_blobs(root_folder_path):
    ...
    # 获取根目录的基准名称
    root_name = os.path.basename(os.path.abspath(root_folder_path))
    
    try:
        # 遍历一级子目录
        for first_level_item in os.listdir(root_folder_path):
            if "lidar" in first_level_item:
                continue
...
def mask_or_random_replace_tokens(image_tokens, mask_id, config, mask_schedule, is_train=True, init_condition_time=0.9, cond_time_divid=10):
    ...
    if not is_train and config.training.get("eval_mask_ratios", None):
        ...
        # Sample a random timestep for each image,control mask ratio range
        timesteps = torch.rand(batch_size, device=image_tokens.device)
        # timesteps_cond = init_condition_time+torch.rand(batch_size, device=image_tokens.device)/cond_time_divid
        # Sample a random mask probability for each image using timestep and cosine schedule
        mask_prob = mask_schedule(timesteps)
        # mask_prob_cond = mask_schedule(timesteps_cond)
        mask_prob = mask_prob.clip(config.training.min_masking_rate)
        # mask_prob_cond = mask_prob_cond.clip(config.training.min_masking_rate)
    # creat a random mask for each image
    num_token_masked = (seq_len * mask_prob).round().clamp(min=1)
    # num_token_masked_cond = (seq_len * mask_prob_cond).round().clamp(min=1)
...
def image_transform(image, resolution=None, normalize=True):
    # image = transforms.Resize(resolution, interpolation=transforms.InterpolationMode.BICUBIC)(image)
    if resolution is None:
        image = transforms.Resize((480, 256), interpolation=transforms.InterpolationMode.BICUBIC)(image)
    else:
        image = transforms.Resize(resolution, interpolation=transforms.InterpolationMode.BICUBIC)(image)
