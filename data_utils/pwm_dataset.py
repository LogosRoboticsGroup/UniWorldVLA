...
import os
from training.utils import adapt_raw_scene_blobs, get_config, flatten_omega_conf, image_transform
...
class DatasetNavsim(Dataset):
    def __init__(self,
                ...
                # 深度字段提取控制参数
                default_depth_fields=['depth'],  # 默认提取的字段为depth
                enable_depth_validation=True,    # 是否启用有效性检查
                extract_depth_fields=None,       # 自定义提取的字段列表，None时使用默认
                ):
        super(DatasetNavsim, self).__init__()
        self.split = split #train or test
        self.navsim_root = config.dataset.navsim_root
        os.environ["NUPLAN_MAPS_ROOT"] = os.path.join(config.dataset.navisim_maps, "maps")
        ...
        # 深度缓存配置 - DataLoader只负责加载
        depth_cache_config = getattr(config.dataset, 'depth_cache', {})
        self.depth_cache_config = depth_cache_config
        self.use_depth_cache = self.depth_cache_config.get('use_cached_depth', False)
        self.depth_cache_dir = self.depth_cache_config.get('cache_dir', './depth_cache/navsim')
        ...
        # 初始化深度字段提取控制参数
        self.default_depth_fields = default_depth_fields
        self.enable_depth_validation = enable_depth_validation
        self.extract_depth_fields = extract_depth_fields if extract_depth_fields is not None else default_depth_fields
        ...
        self.scene_loader = SceneLoader(
            ...
            back_up_dict=self.scene_index_dict
        )
        ...
        self.nuplan_10hz_blobs = config.dataset.nuplan_10hz_blobs

        # 初始化相关属性 - 直接从dataset配置获取
        ...
        self.scene_folder_dict = adapt_raw_scene_blobs(os.path.join(self.nuplan_10hz_blobs, 'train'))
        self.scene_folder_dict.update(adapt_raw_scene_blobs(os.path.join(self.nuplan_10hz_blobs, 'test')))
        self.scene_folder_dict.update(adapt_raw_scene_blobs(os.path.join(self.nuplan_10hz_blobs, 'val')))

        # Initialize DepthAnything3 model for depth processing
        self.depth_model = DepthAnything3(model_name="da3-large",load_model=False)

        # 计算交集并保持原有顺序
        intersection = set(self.metric_cache_loader.tokens) & set(self.scene_loader.tokens)
        self.temp_metric_cache_loader = [
            token for token in self.metric_cache_loader.tokens if token in intersection
        ]
        self.have_searched_first_deepth = False
        self.scene_index_dict = self.load_scene_index_pkl()
        
    def __len__(self):
        # return len(self.metric_cache_loader.tokens)
        return len(self.temp_metric_cache_loader)

    def __getitem__(self, idx):
        while True:
            try:
                return_data = self.getitem(idx)
                if bool(int(os.getenv("RUN_FLASH_DATA_LOADER", 0))):
                    distance = 1 if self.have_searched_first_deepth else 500
                    if isinstance(return_data,bool):
                        # if  return_data["cached_prev_depth"] !={} and return_data["cached_next_depth"] !={}:
                        print("full data skip")
                        self.have_searched_first_deepth = False
                        idx += distance
                        idx %= self.__len__()
                        continue
                    else:
                        self.have_searched_first_deepth = True
                        return return_data
                return return_data
    ...
    def getitem(self, idx):
        ...
        token = self.temp_metric_cache_loader[idx]
        ...
        prev_img, next_img, prev_tokenids, next_tokenids_temp = self._get_images(token)
        if bool(int(os.getenv("RUN_FLASH_DATA_LOADER", 0))):
            if self.check_cache_validity(prev_tokenids[0]):
                return True
            
        # is_missing_next_img = len(next_img) < self.next_frames
        # if is_missing_next_img:
        #     next_img += [next_img[-1].copy() for _ in range(self.next_frames - len(next_img))]
        #     next_tokenids_temp += [next_tokenids_temp[-1].copy() for _ in range(self.next_frames - len(next_img))]
        # full_next_img = next_img.copy()
        # next_img = next_img[4::5]
        # # next_tokenids = next_tokenids_temp[4::5]
        # images_prev_miss, images_next_miss = self.prev_frames - len(
        #     prev_img
        # ), self.next_frames - len(next_img)
        # assert images_prev_miss >= 0 and images_next_miss >= 0
        
        # full_next_img = [prev_img[-2], prev_img[-1], *full_next_img]
        ...
        # # next_img_context, next_img_dynamic = self.data_augmentation(next_img)
        # next_img_context_per_sec = []
        # for i in range(6,42,10): #对应2Hz位置指标
        #     tmp_ids = (i-6,i-5,i,i+5) 
        #     # -0.1 0 0.5 1.0 / 0.9 1.0 1.5 2.0 / 1.9 2.0 2.5 3.0 / 2.9 3.0 3.5 4.0
        #     next_img_context_tmp, _ = self.data_augmentation([full_next_img[j] for j in tmp_ids])
        #     next_img_context_per_sec.append(next_img_context_tmp)
        # next_img_context_per_sec = torch.stack(next_img_context_per_sec, dim=0)
        # 处理图像数据流程
        (prev_img_context_1s, prev_img_dynamic_1s,
         prev_img_context_2s, prev_img_dynamic_2s,
         next_img_context,  next_img_dynamic,next_img_fill,next_tokenids_fill,next_img_context_tokenids_per_sec,next_img_context_img) = self.process_data_with_augmentation_8frame_05s(
            prev_img, next_img, self.data_augmentation,next_tokenids_temp,prev_tokenids
        )
         
         # 检查并调整深度图像和token ids的长度
        prev_img_depth = self.check_depth_len(prev_img, self.fps * 2)
        prev_tokenids = self.check_depth_len(prev_tokenids, self.fps * 2)
        ...
        # Process depth data with caching support using parameterized field extraction
        (
            processed_prev,
            processed_next,
        ...
        ) = self.__load_depth_data_with_cache(
            token,
            prev_img_depth,
            next_img_fill[2:],
            next_img_context=next_img_context_img,
            extract_fields=self.extract_depth_fields,
            prev_tokenids=prev_tokenids,
            next_tokenids=next_tokenids_fill,
            next_tokenids_context=next_img_context_tokenids_per_sec,
        )
        
        (prev_depth_context_1s, prev_depth_dynamic_1s,
            prev_depth_context_2s, prev_depth_dynamic_2s,
            next_depth_context, next_depth_dynamic) = self._process_depth_to_contex_and_dynamic(cached_prev_depth, cached_next_depth,cached_next_depth_context)

        return self.AgentInput(
            token=token,
            ...
            # next_img_context_per_sec=next_img_context,
            scene_log=scene_log,
            ...
            cached_next_depth_context=cached_next_depth_context,
            extrinsics=extrinsics,
            intrinsics=intrinsics,
            processed_prev=processed_prev,
            processed_next=processed_next,
            processed_next_context=processed_next_context,
            prev_tokenids=prev_tokenids,
            next_tokenids=next_tokenids_fill, 
            next_context_tokenids=next_img_context_tokenids_per_sec,         
        )
    def _process_depth_to_contex_and_dynamic(self, cached_prev_depth, cached_next_depth,cached_next_depth_context=None):
        """从缓存处理深度数据，分割为context和dynamic部分
        
        Args:
            cached_prev_depth: 之前的缓存深度数据
            cached_next_depth: 之后的缓存深度数据
            
        Returns:
            tuple: (prev_depth_context_1s, prev_depth_dynamic_1s,
                    prev_depth_context_2s, prev_depth_dynamic_2s,
                    next_depth_context, next_depth_dynamic)
        """
        prev_depth_list = []
        next_depth_list = []
        context_next_depth_list = []
        if cached_prev_depth and 'depth' in cached_prev_depth:
            prev_depth_tensor = cached_prev_depth['depth']
            # 将合并后的tensor转换为列表，每个元素是[1, H, W]
            if len(prev_depth_tensor.shape) == 3:  # [H, W, T] -> T个[1, H, W]
                for i in range(prev_depth_tensor.shape[0]):
                    depth_frame = prev_depth_tensor[i, :, :].unsqueeze(0)
                    prev_depth_list.append(depth_frame)
            elif len(prev_depth_tensor.shape) == 2:  # [H, W]
                prev_depth_list.append(prev_depth_tensor.unsqueeze(0))
        
        if cached_next_depth and 'depth' in cached_next_depth:
            next_depth_tensor = cached_next_depth['depth']
            # 将合并后的tensor转换为列表
            if len(next_depth_tensor.shape) == 3:
                for i in range(next_depth_tensor.shape[0]):
                    depth_frame = next_depth_tensor[i, :, :].unsqueeze(0)
                    next_depth_list.append(depth_frame)
            elif len(next_depth_tensor.shape) == 2:
                next_depth_list.append(next_depth_tensor.unsqueeze(0))
                
        if cached_next_depth_context and 'depth' in cached_next_depth_context:
            context_next_depth_tensor = cached_next_depth_context['depth']
            # 将合并后的tensor转换为列表
            if len(context_next_depth_tensor.shape) == 3:
                for i in range(context_next_depth_tensor.shape[0]):
                    depth_frame = context_next_depth_tensor[i, :, :].unsqueeze(0)
                    context_next_depth_list.append(depth_frame)
            elif len(context_next_depth_tensor.shape) == 2:
                context_next_depth_list.append(context_next_depth_tensor.unsqueeze(0))
        # 分割并处理深度数据
        ...
        if prev_depth_list and next_depth_list and context_next_depth_list:
            # 使用通用函数处理深度数据
            (prev_depth_context_1s, prev_depth_dynamic_1s,
             prev_depth_context_2s, prev_depth_dynamic_2s,
             next_depth_context, next_depth_dynamic) = self.process_data_with_augmentation_8frame_depth(
                prev_depth_list, next_depth_list, self.depth_augmentation,context_next_depth_list
            )
        else:
            print("====>数据不够")
        
        return (prev_depth_context_1s, prev_depth_dynamic_1s,
                prev_depth_context_2s, prev_depth_dynamic_2s,
                next_depth_context, next_depth_dynamic)
    ...
    def _init_depth_cache(self):
        ...
        # 初始化缓存索引
        self.depth_index_file = os.path.join(self.depth_cache_dir, "depth_index.json")
        if os.path.exists(self.depth_index_file):
            with open(self.depth_index_file, 'r') as f:
                self.depth_index = json.load(f)
        else:
            self.depth_index = {}

    def _get_depth_cache_path(self, token_id: str, is_prev: bool = True) -> str:
        """根据token id生成深度数据缓存文件路径"""
        return get_depth_cache_path(
    ...
    def _load_depth_data_from_cache(self, token_id: str, is_prev: bool = True, validation_fields=None):
        """从缓存加载深度数据，支持参数化的有效性检查，返回格式化的字段字典"""
        if not self.use_depth_cache:
            return {}
        cache_path = ""
        try:
            import torch
            cache_path = self._get_depth_cache_path(token_id, is_prev=is_prev)
            ...
            fields_to_extract =  self.extract_depth_fields
            extracted_dict = {}
            for field in fields_to_extract:                              
                prediction_data = cached_data['prediction_data']
                if field in prediction_data:
                    extracted_dict[field] = prediction_data[field]

            return extracted_dict
        except Exception as e:
            os.remove(cache_path)
            print(f"加载深度数据失败, token_id: {token_id}, is_prev: {is_prev}, error: {e}")
            return {}
        # # self.scene_folder_dict = adapt_raw_scene_blobs(os.path.join(self.nuplan_10hz_blobs, 'train'))
        # # self.scene_folder_dict.update(adapt_raw_scene_blobs(os.path.join(self.nuplan_10hz_blobs, 'test')))
        # # self.scene_folder_dict.update(adapt_raw_scene_blobs(os.path.join(self.nuplan_10hz_blobs, 'val')))
        
        # token_intersection = set(self.metric_cache_loader.tokens) & set(self.scene_loader.tokens)
        # self.temp_metric_cache_loader = [token for token in self.metric_cache_loader.tokens if token in token_intersection]
    ...
    # def load_scene_index_pkl(self):
    #     # 加载nuplan_scene_blobs映射字典
    #     root = os.path.join(self.navsim_root, "scene_index.pkl")
    #     if not os.path.exists(root):
    #         raise RuntimeError
    #     with open(root, "rb") as f:
    #         scene_index_dict = pickle.load(f)
    #     return scene_index_dict
    ...
    def AgentInput(self, 
        ...
        return dict(token=token,
                    ...
                    extrinsics=extrinsics,
                    intrinsics=intrinsics,
                    next_context_tokenids=next_context_tokenids,
                    processed_next_context=processed_next_context,
                    cached_next_depth_context=cached_next_depth_context,
                    )
    def load_scene_index_pkl(self):
    ...
    def _get_images(self, token):
        ...
        for view_prev, view_next in zip(images_prev_info, images_next_info):
            if len(view_prev) >= len_prev_max:
                for i in range(len_prev_max):
                    filename = view_prev[i-len_prev_max]['filename_jpg']
                    prev_img.append(self._load_image(self.nuplan_10hz_blobs, self.nuplan_10hz_name, filename))
                    tokenid = os.path.basename(filename).split('.')[0]
                    prev_tokenids.append(tokenid)
            else:
                for i in range(len(view_prev)):
                    filename = view_prev[i - len(view_prev)]['filename_jpg']
                    prev_img.append(self._load_image(self.nuplan_10hz_blobs, self.nuplan_10hz_name, filename))
                    tokenid = os.path.basename(filename).split('.')[0]
                    prev_tokenids.append(tokenid)
            if len(view_next) >= len_next_max:
                for i in range(len_next_max):
                    filename = view_next[i]['filename_jpg']
                    next_img.append(self._load_image(self.nuplan_10hz_blobs, self.nuplan_10hz_name, filename))
                    tokenid = os.path.basename(filename).split('.')[0]
                    next_tokenids.append(tokenid)
            else:
                for i in range(len(view_next)):
                    ...
                    tokenid = os.path.basename(filename).split('.')[0]
                    next_tokenids.append(tokenid)
        return prev_img, next_img, prev_tokenids, next_tokenids
    ...
    def depth_augmentation(self, depth_images):
        """
        对深度图像进行数据增强，分离context和dynamic
        
        Args:
            depth_images: 深度图像列表，每个是[1, H, W]的tensor
            
        Returns:
            tuple: (context_depths, dynamic_depths)
        """
        con_len_count = 0
        new_depths_context = []
        new_depths = []
        ...
        for depth_img in depth_images:
            # context: 前2帧，低分辨率
            if con_len_count in [0, 1]:
                depth = transforms.Resize((self.Con_resolution_h, self.Con_resolution_w), interpolation=transforms.InterpolationMode.BICUBIC)(depth_img)
                depth = transforms.CenterCrop((self.Con_resolution_h, self.Con_resolution_w))(depth)
                new_depths_context.append(depth)
            
            # dynamic: 所有帧，正常分辨率
            depth = transforms.Resize((self.resolution_h, self.resolution_w), interpolation=transforms.InterpolationMode.BICUBIC)(depth_img)
            depth = transforms.CenterCrop((self.resolution_h, self.resolution_w))(depth)
            new_depths.append(depth)
            con_len_count += 1
            
        return torch.stack(new_depths_context, 0) if new_depths_context else None, torch.stack(new_depths, 0) if new_depths else None
    
    def check_len(self,prev_img_dynamic_2s, dest_frames=10):
        missing_frames = dest_frames - prev_img_dynamic_2s.shape[0]
        if missing_frames > 0:
    ...
    def process_data_with_augmentation_8frame_05s(self, prev_data, next_data, augmentation_fn,next_tokenids_temp,prev_tokenids_temp):
        """通用数据处理流程，支持自定义增强函数
        
        Args:
            prev_data: 之前的数据列表
            next_data: 之后的数据列表
            augmentation_fn: 数据增强函数
            
        Returns:
            tuple: (prev_context_1s, prev_dynamic_1s,
                    prev_context_2s, prev_dynamic_2s,
                    next_context, torch.Size([4, 2, 3, 256, 448])
                    next_dynamic)  torch.Size([9, 3, 128, 224])
                    next_img, + 前历史帧的 img shape 10
                    next_tokenids, 没有历史帧的 对应token id  shape 8
                    next_img_context_tokenids_per_sec, context 的id  shape 4*4
                    next_img_context_img，context，原始 img  shape 4*4
        """
        assert len(next_data) == len(next_tokenids_temp), f"next_data的长度应该是{len(next_data)},next_tokenids_temp的长度应该是 {len(next_tokenids_temp)}"
        # next_img = [prev_data[-2], prev_data[-1], *next_data]
        next_img = deepcopy(next_data)
        next_tokenids = deepcopy(next_tokenids_temp)
        # 对前1秒的数据进行增强
        prev_context_1s, prev_dynamic_1s = augmentation_fn(
            prev_data[: self.fps]
        )
        # 对前2秒的数据进行增强
        prev_context_2s, prev_dynamic_2s = augmentation_fn(
            prev_data[self.fps :]
        )
        # 对next_data进行增强
        # 添加当前观察帧到next_img
        is_missing_next_img = len(next_img) < self.next_frames
        if is_missing_next_img:
            print(f"is_missing_next_img: {is_missing_next_img}")
            next_img += [next_img[-1].copy() for _ in range(self.next_frames - len(next_img))]
            next_tokenids += [deepcopy(next_tokenids_temp[-1]) for _ in range(self.next_frames - len(next_tokenids_temp))]
        assert len(next_img) == len(next_tokenids), f"next_img的长度应该是{len(next_img)},next_tokenids的长度应该是 {len(next_tokenids)}"
        full_next_img = deepcopy(next_img)
        full_next_img_tokenids = deepcopy(next_tokenids)
        next_img = next_img[4::5]
        next_tokenids = next_tokenids[4::5]
        
        next_img = [prev_data[-2], prev_data[-1], *next_img]        
        _, next_dynamic = augmentation_fn(next_img)
        
        
        full_next_img = [prev_data[-2], prev_data[-1], *full_next_img]
        next_tokenids_context = [prev_tokenids_temp[-2], prev_tokenids_temp[-1], *full_next_img_tokenids]
        assert len(next_tokenids_context) == len(full_next_img), f"next_tokenids_context的长度应该是{len(next_tokenids_context)},full_next_img 的长度应该是 {len(full_next_img)}"
        # prev_img_context_1s, prev_img_dynamic_1s = self.data_augmentation(prev_img[:self.fps])
        # prev_img_context_2s, prev_img_dynamic_2s = self.data_augmentation(prev_img[self.fps:])
        # next_img_context, next_img_dynamic = self.data_augmentation(next_img)
        next_img_context_per_sec = []
        next_img_context_tokenids_per_sec = []
        next_img_context_img = []
        for i in range(6,42,10): #对应2Hz位置指标
            # tmp_ids = (i-6,i-5,i,i+5) 
            tmp_ids = (i-6,i-5) 
            # -0.1 0 0.5 1.0 / 0.9 1.0 1.5 2.0 / 1.9 2.0 2.5 3.0 / 2.9 3.0 3.5 4.0
            next_img_context_tmp, _ = self.data_augmentation([full_next_img[j] for j in tmp_ids])
            next_img_context_per_sec.append(next_img_context_tmp)
            next_img_context_tokenids_per_sec.append([next_tokenids_context[j] for j in tmp_ids])
            next_img_context_img.append([full_next_img[j] for j in tmp_ids])
        next_img_context_per_sec = torch.stack(next_img_context_per_sec, dim=0)
        
        # 检查并调整2秒动态数据的长度
        prev_dynamic_2s = self.check_len(prev_dynamic_2s, self.fps)
        
        # 跳过一帧历史数据
        next_dynamic = next_dynamic[1:]
        
        return (prev_context_1s, prev_dynamic_1s,
                prev_context_2s, prev_dynamic_2s,
                next_img_context_per_sec, next_dynamic,next_img,next_tokenids,next_img_context_tokenids_per_sec,next_img_context_img)


    def process_data_with_augmentation(self, prev_data, next_data, augmentation_fn):
        """通用数据处理流程，支持自定义增强函数
        ...
        # 对next_data进行增强
        # 添加当前观察帧到next_img
        next_context, next_dynamic = augmentation_fn(next_img)
        # if context_next_depth_list:
        #     next_context,_ = augmentation_fn(context_next_depth_list)
    ...
    # def check_depth_len(self,prev_img, dest_frames=20):
    #     prev_img_pre_half = prev_img[0:dest_frames//2]
    #     half_len = dest_frames // 2
    #     padded_frames = self.check_len(prev_img[half_len:], half_len)
    #     prev_img_pre = prev_img_pre_half + padded_frames
    #     return   prev_img_pre

    def check_depth_len(self, prev_img, dest_frames=20):
        # print(f"\nInput frames: {len(prev_img)}, dest_frames: {dest_frames}")
        missing_frames = dest_frames - len(prev_img)
        if missing_frames > 0 and prev_img:
            # 确保有足够的帧用于填充（至少10帧）
            if len(prev_img) >= 11:
                # 获取第10帧（索引9）作为填充帧
                fill_frame = prev_img[10]
                # print(f"Inserting {missing_frames} copies of frame 10 after position 10")
                # 在第10帧后插入填充帧
                for _ in range(missing_frames):
                    prev_img.insert(10, fill_frame)

        return prev_img

    def _load_image(self, root, filesplit, imgname):
        vel_time_floder = imgname.split("/")[0]
        for split in filesplit:
            plus_dir = []
            try:
                plus_dir = self.scene_index_dict[vel_time_floder][0].split("/")[0]
            except KeyError:
                # 处理键不存在的逻辑
                print(f"键 {vel_time_floder} 不存在")
            if plus_dir != split:
                # print(f"输入类型 {plus_dir} 和预期类型{split} 不匹配")
                continue
            img_path = os.path.join(
                root, self.scene_index_dict[vel_time_floder][0], imgname
            )
    ...
    def _process_depth_data(self, images, extract_fields=None):
        """
        处理深度数据，支持参数化的字段提取
        
        Args:
            images: 输入图像
            extract_fields: 要提取的字段列表，默认使用self.extract_depth_fields
        
        Returns:
            tuple: (处理后的深度数据, 外参, 内参)
        """
        # 使用指定的字段或默认字段
        fields_to_extract = extract_fields if extract_fields is not None else self.extract_depth_fields

        extrinsics = None
        intrinsics = None
        process_res = (self.Con_resolution_h, self.Con_resolution_w)
        process_res_method = "upper_bound_resize"

        try:
            with torch.no_grad():
                # Use the model's input processor directly
                imgs_cpu, extrinsics, intrinsics = self.depth_model._preprocess_inputs(
                    images, extrinsics, intrinsics, process_res, process_res_method, num_workers=1
                )

                # 根据extract_fields返回相应的数据
                if 'depth' in fields_to_extract:
                    return imgs_cpu, extrinsics, intrinsics
                elif 'extrinsics' in fields_to_extract:
                    return None,extrinsics, None
                elif 'intrinsics' in fields_to_extract:
                    return  None, None,intrinsics
                else:
                    # 默认返回深度数据
                    return imgs_cpu, extrinsics, intrinsics
            ...
            raise
            # Return None or a default tensor if processing fails
            return None, None, None

    def _merge_depth_tensors(self, tensor_dicts: List[Dict[str, torch.Tensor]], fields: List[str]) -> Dict[str, torch.Tensor]:
        """
        合并多个字典中相同字段的tensor
        
        Args:
            tensor_dicts: 包含tensor的字典列表
            fields: 需要合并的字段列表
        ...
        for field in fields:
            ...
            for d in tensor_dicts:
                if field in d and isinstance(d[field], torch.Tensor):
                    tensors.append(d[field])
            if tensors:
                merged[field] = torch.stack(tensors, dim=2).squeeze()
        return merged

    def check_cache_validity(self, token_id,):
        depth = self._load_depth_data_from_cache(
            token_id, is_prev=True, validation_fields=self.extract_depth_fields
        )
        return depth
    ...
    def __load_depth_data_with_cache(self, token_id: str, prev_img, next_img, next_img_context, extract_fields=None,
                                     prev_tokenids: Optional[List[str]] = None,
                                     next_tokenids: Optional[List[str]] = None,
                                     next_tokenids_context: Optional[List[str]] = None):
        """
        处理深度数据，支持缓存加载和参数化字段提取
        ...
        Args:
            ...
            next_img: 之后的图像
            extract_fields: 要提取的字段列表，默认使用self.extract_depth_fields
            next_img_context_tokenids_per_sec: 每秒的图像上下文tokenids
        Returns:
            tuple: (缓存的prev数据, 缓存的next数据, 外参, 内参)
        """
        # 使用指定的字段或默认字段
        fields_to_extract = extract_fields if extract_fields is not None else self.extract_depth_fields
        ...
        extrinsics = None
        intrinsics = None
        cached_next_depth_context = None
        processed_next_context = None
        # 尝试从缓存加载原始数据结构
        if self.use_depth_cache:
            # 确保tokenids是列表类型
            prev_ids =  prev_tokenids
            next_ids =  next_tokenids

            # 加载多个token的缓存数据
            cached_prev_depths = []
            cached_next_depths = []
            cached_next_depths_context = []
            depth = {}
            for pid in prev_ids:
                depth = self._load_depth_data_from_cache(
                    pid,
                    is_prev=True,
                    validation_fields=fields_to_extract
                )
                if depth:
                    cached_prev_depths.append(depth)
                else:
                    cached_prev_depths = []
                    break

            if len(cached_prev_depths)>0:
                for nid in next_ids:
                    depth = self._load_depth_data_from_cache(
                        nid,
                        is_prev=False,
                    ...
                    if depth:
                        cached_next_depths.append(depth)
                    else:
                        cached_next_depths = []
                        break
            if len(cached_prev_depths)>0 and len(cached_next_depths)>0 :
                for nid_list in next_tokenids_context:
                    for nid in nid_list:
                        depth = self._load_depth_data_from_cache(
                            nid,
                            is_prev=False,
                        ...
                        if depth:
                            cached_next_depths_context.append(depth)
                        else:
                            cached_next_depths_context = []
                            break
                    if len(cached_next_depths_context)==0:
                        break
            # 合并相同字段的tensor
            cached_prev_depth = self._merge_depth_tensors(cached_prev_depths, fields_to_extract) if cached_prev_depths else {}
            cached_next_depth = self._merge_depth_tensors(cached_next_depths, fields_to_extract) if cached_next_depths else {}
            cached_next_depth_context = self._merge_depth_tensors(cached_next_depths_context, fields_to_extract) if cached_next_depths_context else {}
            processed_prev, extrinsics, intrinsics = self._process_depth_data(prev_img, fields_to_extract)
            processed_next, _, _ = self._process_depth_data(next_img, fields_to_extract)
            processed_next_context_list = []
            for i in range(len(next_img_context)):
                processed_next_context_i, _, _ = self._process_depth_data(next_img_context[i], fields_to_extract)
                processed_next_context_list.append(processed_next_context_i)
            processed_next_context = torch.stack(processed_next_context_list, dim=0)
            if not cached_next_depth  or not cached_prev_depth or not cached_next_depth_context:
                # 缓存未命中，仍然进行预处理，使用参数化字段提取
                pass      
            else:
                # print("成功加载缓存数据==》"+str(token_id))
                pass
        return processed_prev, processed_next,processed_next_context, extrinsics, intrinsics, cached_prev_depth, cached_next_depth, cached_next_depth_context

    def process_data_with_augmentation_8frame_depth(self, prev_data, next_data, augmentation_fn,context_next_depth_list=[]):
        """通用数据处理流程，支持自定义增强函数
        ...
        _, next_dynamic = augmentation_fn(next_img)
        next_context_list = []
        for i in range(len(context_next_depth_list)//2):
            next_context,_ = augmentation_fn(context_next_depth_list[2*i:2*(i+1)])
            next_context_list.append(next_context)
        next_context_reshape = torch.stack(next_context_list, dim=0)
        # 检查并调整2秒动态数据的长度
        prev_dynamic_2s = self.check_len(prev_dynamic_2s, self.fps)
        ...
        return (prev_context_1s, prev_dynamic_1s,
                prev_context_2s, prev_dynamic_2s,
                next_context_reshape, next_dynamic)
...
class DataCollatorForNavsim(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        prev_tokenids = [instance["prev_tokenids"] for instance in instances]
        next_tokenids = [instance["next_tokenids"] for instance in instances]
        next_context_tokenids = [instance["next_context_tokenids"] for instance in instances]
        batch = dict(
            token=token,
            scene_log=scene_log,
            prev_tokenids=prev_tokenids,
        ...
        for i in ['prev_img_context_1s', 'prev_img_dynamic_1s',
                  ...
                  "cached_prev_depth","cached_next_depth",
                    "processed_next_context",
                    "cached_next_depth_context"]:
            if i in instances[0]:
                list_insts = [instance[i] for instance in instances]
                
                # 如果 list_insts 中有一个为空字典或None，则 continue
                if not list_insts or any(isinstance(inst, dict) and not inst for inst in list_insts):
                    continue

                # Handle two-layer dict structure for cached depth data
                if i in ["cached_prev_depth", "cached_next_depth","cached_next_depth_context"] and isinstance(list_insts[0], dict):
                    # For dictionary format, we need to handle each field separately
                    try:
                        batch[i] = {}
                        for field_name in list_insts[0].keys():
                            field_values = []
                            for inst in list_insts:
                                if isinstance(inst, dict) and field_name in inst:
                                    field_value = inst[field_name]
                                    # Handle nested lists or individual tensors
                                    if type(field_value) is list:
                                        for item in field_value:
                                            field_values.append(item)
                                    else:
                                        field_values.append(field_value)
                            # Stack if all tensors have the same shape
                            if field_values and all(x is not None and hasattr(x, 'shape') and x.shape == field_values[0].shape for x in field_values):
                                batch[f"{i}"][field_name] = torch.stack(field_values)
                            else:
                                batch[f"{i}"][field_name] = {}
                    except Exception as e:
                        print(f"Error processing {i}: {e}")
                        raise
                else:
                    # Original logic for non-dict data
                    Batchwise_inst = []
                    for inst in list_insts:
                        if type(inst) is list:
                            for item in inst:
                                Batchwise_inst.append(item)
                        else:
                            Batchwise_inst.append(inst)

                    # Stack if all tensors have the same shape
                    if Batchwise_inst and all(x is not None and hasattr(x, 'shape') and x.shape == Batchwise_inst[0].shape for x in Batchwise_inst):
