...
#     SCENE_INDEX: Dict[str, str] = pickle.load(f)
# e.g. SCENE_INDEX["2021.08.24.17.34.27_veh-45_00808_00993"]=="train/nuplan-v1.1_train_camera_0"
config = OmegaConf.load("configs/sft_navsim/navsim.yaml")
        ...
        for camera_name in camera_dict.keys():
            ...
            if camera_identifier in sensor_names:
                ...
                if not os.path.exists(image_path):
                    tmp_path_split = camera_dict[camera_name]["data_path"].split("/")
                    replace_stamp = back_up_dict[tmp_path_split[0]][0]
                    image_path = Path(
                        os.path.join(
                            config.dataset.nuplan_10hz_blobs,
                            replace_stamp,
                            *tmp_path_split,
                        )
                    )
...
class Lidar:
    ...
    @classmethod
    def from_paths(cls, sensor_blobs_path: Path, lidar_path: Path, sensor_names: List[str], back_up_dict: Dict) -> Lidar:
        """
        ...
        if "lidar_pc" in sensor_names:
            ...
            if not os.path.exists(global_lidar_path):
                tmp_path_split = str(lidar_path).split("/")  # 2021.xxx_veh-xxx
                replace_stamp = back_up_dict[tmp_path_split[0]][
                    1
                ]  # val/nuplan-v1.1_val_lidar_0
                global_lidar_path = Path(
                    os.path.join(
                        config.dataset.nuplan_10hz_blobs, replace_stamp, *tmp_path_split
                    )
                )
                # print("恢复lidar")
...
class SceneFilter:
    ...
    frame_interval: Optional[int] = 1 #None
    has_route: bool = True
...
class SensorConfig:
    ...
    @classmethod
    def build_front_sensors(cls, include: Union[bool, List[int]] = True) -> SensorConfig:
        """
        Classmethod to load all sensors with the same specification.
        :param include: boolean or integers for sensors to include, defaults to True
        :return: sensor configuration dataclass
