...
def add_lidar_to_camera_ax_with_depth(
    ...
    # 将 LIDAR 点投影到图像平面并绘制
    for (x, y), depth, color in zip(
        pc_in_cam[:, 0:2], depth_values, lidar_pc_colors
    ):  # 只提取 (x, y)
        ...
        if 0 <= x < image_width and 0 <= y < image_height:
            color = (int(color[0]), int(color[1]), int(color[2]))
            cv2.circle(image, (int(x), int(y)), 5, color, -1)
            depth_map_intermediate[y, x] = min(
                depth_map_intermediate[y, x], depth
            )  # 更新深度图
    ...
    # 归一化深度图到 0-255 范围
    dense_depth_map_normalized = cv2.normalize(
        dense_depth_map, None, 0, 255, cv2.NORM_MINMAX
    )

    # 使用 'Spectral_r' 颜色映射应用于密集深度图
    colormap = plt.get_cmap("Spectral_r")
    colored_dense_depth_map = colormap(
        dense_depth_map_normalized / 255.0
    )  # 归一化后映射到 [0, 1]
    ...
    # 保存密集深度图
    cv2.imwrite(
        os.path.join(outdir, filename + "_dense_depth.png"), colored_dense_depth_map
    )
...
def add_2d_annotations_to_camera_ax(
    ax: plt.Axes, camera: Camera, annotations: Annotations
) -> plt.Axes:
    """
...
def _transform_pcs_to_images_with_depth(
    ...
    pc_xyz = lidar_pc[
        LidarIndex.POSITION, :
    ].T  # shape should be (n, 3), each row [x, y, z]
    ...
    cur_pc_xyz = np.concatenate(
        [pc_xyz, np.ones_like(pc_xyz)[:, :1]], axis=-1
    )  # shape: (n, 4)
    print(
        f"cur_pc_xyz shape after concat: {cur_pc_xyz.shape}"
    )  # Debug: Check shape after concat

    # 投影到相机坐标系
    cur_pc_cam = lidar2img_rt @ cur_pc_xyz.T  # 投影矩阵 * 点云
    cur_pc_cam = cur_pc_cam.T  # 转置回来 (n, 4)
    print(
        f"cur_pc_cam shape after projection: {cur_pc_cam.shape}"
    )  # Debug: Check shape after projection

    # 过滤掉视野外的点
    cur_pc_in_fov = cur_pc_cam[:, 2] > eps  # z > eps 才是有效点

    # 进行齐次坐标的标准化：通过 Z 值标准化 (x, y)，得到二维坐标
    cur_pc_cam[:, 0:2] = cur_pc_cam[:, 0:2] / np.maximum(
        cur_pc_cam[:, 2:3], np.ones_like(cur_pc_cam[:, 2:3]) * eps
    )
    print(
        f"cur_pc_cam shape after normalization: {cur_pc_cam.shape}"
    )  # Debug: Check shape after normalization

    # 获取深度值（即 Z 坐标）
    depth_values = cur_pc_cam[:, 2]  # 深度值为 Z 坐标
    print(
        f"depth_values shape: {depth_values.shape}"
    )  # Debug: Check depth_values shape
...
def add_trajectory_to_camera_ax(
    ax: plt.Axes, camera: Camera, trajectory: Trajectory, config: Dict[str, Any], params
) -> plt.Axes:
    """
    ...
    poses_2d = trajectory[
        :, :2
    ]  # 获取 x, y，假设 trajectory.poses 是 [x, y, heading]
    poses_3d = np.concatenate(
        [poses_2d, np.zeros((poses_2d.shape[0], 1))], axis=1
    )  # 添加 z=0 列
    poses = np.concatenate(
        [np.array([[0, 0, 0]]), poses_3d]
    )  # 假设轨迹起始点在 (0,0,0)
    ...
    second_point = (
        pc_in_cam[2] if len(pc_in_cam) > 2 else None
    )  # 第二个轨迹点投影后的点
    ...
    if first_point is not None and pc_in_fov_mask[1]:
        ...
        intersection_point = _get_intersection_with_image_bottom_boundary(
            start_pt, end_pt, image_width, image_height
        )
    ...
    if len(points_to_plot) > 1:  # 确保至少有两个点来画线
        ...
        #for traj to project to 3view img
        # plot_points[:, 0] = (plot_points[:, 0] + params[0]) * params[1]
        # plot_points[:, 1] = (plot_points[:, 1] - 28) * params[2]
...
def _get_intersection_with_image_bottom_boundary(
    origin_point, end_point, image_width, image_height
):
    """
    计算线段与图像*底部*边界的交点。
    ...
    if y1 != y2:  # 避免除以零
        ...
        if x_min <= x_intersection_bottom <= x_max and min(y1, y2) <= y_max <= max(
            y1, y2
        ):  # 检查交点x坐标在图像宽度内，且y_max在y1, y2之间
            return np.array([x_intersection_bottom, y_max])

    return None  # 没有与底部边界的有效交点


def lidar_to_camera_point(lidar_point, sensor2lidar_rotation, sensor2lidar_translation):
    """
    将一个 LiDAR 坐标系下的点转换到相机坐标系
    """
    lidar_point = np.array(lidar_point)
    point_h = np.concatenate([lidar_point, [1.0]])  # 齐次坐标 (x,y,z,1)

    # 计算 LiDAR → Camera 转换矩阵
    lidar2cam_r = np.linalg.inv(sensor2lidar_rotation)
    lidar2cam_t = sensor2lidar_translation @ lidar2cam_r.T
...
def visualize_pred_gt_camera_bev(
    camera,
    pred_point_lidar: np.ndarray,
    gt_point_lidar: np.ndarray,
    ego_length: float = 4.5,
    ego_width: float = 2.0,
    save_path: str = None
):
    """
    可视化预测点和GT点: 左侧相机投影, 右侧BEV视角
    """
    fig, axes = plt.subplots(1, 2, figsize=(12, 7))  # 高度加大给底部留字

    # ==== 左边: 原图视角 ====
    ax_img = axes[0]
    ax_img.imshow(camera.image)
    ax_img.set_title("Camera view")
    ax_img.axis('off')

    # 坐标转换: LiDAR → Camera
    pred_point_cam = lidar_to_camera_point(pred_point_lidar,
                                           camera.sensor2lidar_rotation,
                                           camera.sensor2lidar_translation)
    gt_point_cam = lidar_to_camera_point(gt_point_lidar,
                                         camera.sensor2lidar_rotation,
                                         camera.sensor2lidar_translation)

    # 投影到像素坐标
    pred_pixel, pred_mask = _transform_points_to_image(
        np.array([pred_point_cam]),
        camera.intrinsics,
        image_shape=camera.image.shape[:2]
    )
    gt_pixel, gt_mask = _transform_points_to_image(
        np.array([gt_point_cam]),
        camera.intrinsics,
        image_shape=camera.image.shape[:2]
    )
    ...
    # 绘制预测点（红圈）和GT（绿星）
    if pred_mask[0]:
        ax_img.scatter(pred_pixel[0,0], pred_pixel[0,1],
                       s=60, edgecolors='red', facecolors='none', linewidths=2,
                       label='Prediction')
    if gt_mask[0]:
        ax_img.scatter(gt_pixel[0,0], gt_pixel[0,1],
                       marker='*', s=80, c='green', label='GT')
    ax_img.legend()
    ...
    ax_bev.set_xlim([-30, 30])
    ax_bev.set_ylim([-20, 80])
    ax_bev.grid(True)
    ax_bev.set_aspect('equal')

    # 自车矩形（中心对称）
    car_rect = Rectangle((-ego_width/2, -ego_length/2),
                         ego_width, ego_length,
                         edgecolor='black', facecolor='gray', alpha=0.5)
    ax_bev.add_patch(car_rect)

    # 绘制预测点和GT点
    ax_bev.scatter(pred_point_lidar[1], pred_point_lidar[0],
                   s=60, edgecolors='red', facecolors='none', linewidths=2,
                   label='Prediction')
    ax_bev.scatter(gt_point_lidar[1], gt_point_lidar[0],
                   marker='*', s=80, c='green', label='GT')
    ax_bev.legend()

    # ==== 底部文字信息 ====
    pred_text = (f"Prediction Point (X forward, Y left, Z up): "
    ...
    fig.text(0.5, 0.02, pred_text, ha='center', fontsize=10, color='red')
    fig.text(0.5, 0.00, gt_text, ha='center', fontsize=10, color='green')
...
def visualize_pred_gt_camera_traj(
    camera_,
    pred_point_lidar,
    gt,
    ego_length: float = 4.5,
    ...
    可视化预测点和GT点: 左侧相机投影, 右侧BEV视角
    """
    img, params = _get_camera_feature(camera_)
    fig1, ax1 = plt.subplots(1, 1, figsize=(24, 6))
    ax1.imshow(img)
    ax1.axis('off')
    # plt.savefig(str(save_path).replace(".png", "_front_view.png"), bbox_inches='tight', pad_inches=0)
    # plt.close(fig1)

    # fig, ax0 = plt.subplots(1, 1, figsize=(24, 6))

    # ==== 左边: 原图视角 ====
    ax_img = ax1

    # ax_img.imshow(img)
    # # title = "GT" if type == "gt" else "Prediction"

    # # ax_img.set_title(title, fontsize=16)
    # ax_img.axis('off')

    camera = camera_.cam_f0
    ...
    ax_img = add_trajectory_to_camera_ax(ax_img, camera, gt, trajectory_config_human, params)
    ...
    ax_img = add_trajectory_to_camera_ax(ax_img, camera, pred_point_lidar, trajectory_config_human, params)

    if save_path:
        plt.savefig(save_path, bbox_inches='tight', pad_inches=0)
    plt.show()

def _get_camera_feature(cameras):
    """
    ...
    # 计算缩放比例
    scale_x = target_w / stitched_w
    scale_y = target_h / stitched_h

    resized_image = cv2.resize(stitched_image, (1024, 256))
    tensor_image = transforms.ToTensor()(resized_image)

    return tensor_image.permute(1, 2, 0).cpu().numpy(), [l0_w, scale_x, scale_y]