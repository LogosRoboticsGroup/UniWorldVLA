...
if [ -z "$LPAI_MASTER_0_HOST" ]; then
  ...
  export BATCH_SIZE_VAL_NUS=2
  export LOG_EVERY=50
  export LPAI_ALL_NUM=1
  export LOAD_LX_TRAINING=1
  export EVAL_DIR="/lpai/volumes/ad-agent-vol-ga/xhh/PWM-DA3/pwm-xxh-3ff5889e/checkpoint_step_9121"
fi
if [ "$LPAI_ALL_NUM" -eq 1 ]; then
  NCCL_ALGO=Ring echo "========================>8_gpus_deepspeed_zero2.yaml"
