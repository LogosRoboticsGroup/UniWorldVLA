...
class Compressive_magvit_v2(ModelMixin, ConfigMixin):
    ...
    def tokenize(self,
                 ...
                 context_pixel_values: torch.FloatTensor ,
        ...
        if is_img:
            img_special_token_s = special_token['<|soi|>']
            img_special_token_e = special_token['<|eoi|>']
            img_special_token_s_dynamic = special_token['<|sod|>']#depth
            img_special_token_e_dynamic = special_token['<|eod|>']
        else:
            img_special_token_s = img_special_token_s_dynamic = special_token['<|sop|>']#depth
            img_special_token_e = img_special_token_e_dynamic = special_token['<|eop|>']
        ...
        if not only_context_vq:#dynamic
            ...
            if is_img  :
                ...
                B,T, C, H, W = pixel_values.shape
                context_length = T
                context_frames = pixel_values.reshape(-1, C, H, W)
        # encode context frames
        h, cond_features,for_continues_feat = self.context_vq_model.encoder(context_frames, return_features=True)  # h->(T_current,C,H,W) # Bt,3,H,W->Bt,13,H//16,W//16,h.max=16
        
        # 保存context特征用于后续融合
        context_encoder_features = for_continues_feat
        if return_encoder_features and return_stage ==  1:
            encoder_features = {
                'context': context_encoder_features,
                'dynamic': None
            }
            return None, None, encoder_features 
        ...
        if only_context_vq:
            ...
            c_latent_size = indices_c.shape[-1]
            token_soi = img_special_token_s.to(pixel_values.device)  # special token for context frames
            token_eoi = img_special_token_e.to(pixel_values.device)
            ...
            indices_c = torch.cat([token_soi, indices_c, token_eoi], dim=2).reshape(B, -1)
            if not return_label:
                indices = {
                    "context": indices_c,
                    "dynamic": indices_c[:, : -(2 + c_latent_size)],
                }  #
                labels = {"dynamic": indices_c[:, 2 + c_latent_size :].clone()}
            else:
                dynamic = indices_c[:, 2 + c_latent_size  :]
                indices = {
                    "context": indices_c,
                    "dynamic": dynamic[:, : -(2 + c_latent_size)],
                }
                labels = {"dynamic": dynamic[:, 2 + c_latent_size :].clone()}
            
            if return_encoder_features and return_stage==1:
                # 返回分离的context和dynamic特征（只有context的情况）
                encoder_features = {
                    'context': cond_features,
                    'dynamic': None
                }
                return indices, labels, encoder_features
        ...
        d,feat_d = self.dynamic_vq_model.encoder(future_frames, cond_features, cond=self.cond_enable, attn_mask=None, time_ids=None)  # d.max=23
        # 保存dynamic encoder的原始输出（在patchify之前）
        dynamic_encoder_features = feat_d
        p = self.patch_size
        ...
        token_sod = img_special_token_s_dynamic.to(pixel_values.device)  # special token for context frames
        token_eod = img_special_token_e_dynamic.to(pixel_values.device)
        ...
        if return_encoder_features and return_stage==2:
            # 返回分离的context和dynamic特征，便于后续分别融合
            encoder_features = {
                'context': context_encoder_features,
                'dynamic': dynamic_encoder_features
            }
            return indices, labels, encoder_features
        return indices, labels
    # @torch.no_grad()
    # def get_image_encoder_features(self, 
    #                                pixel_values: torch.FloatTensor,
    #                                context_pixel_values: torch.FloatTensor = None,
    #                                context_length: int = 1):
    #     """
    #     获取图像VAE编码器特征用于与深度特征进行cross-attention交互
        
    #     Args:
    #         pixel_values: 输入图像张量，形状 (B, T, C, H, W) 或 (B, C, H, W)
    #         context_pixel_values: 上下文图像张量
    #         context_length: 上下文长度
            
    #     Returns:
    #         image_encoder_features: 编码器特征列表
    #     """
    #     if context_pixel_values is not None:
    #         B, Tc, C, Hc, Wc = context_pixel_values.shape
    #         context_frames = context_pixel_values.reshape(-1, C, Hc, Wc)
    ...
    #     # 编码并获取特征
    #     _, cond_features = self.context_vq_model.encoder(context_frames, return_features=True)
